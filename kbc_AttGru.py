...
class Encoder(tf.keras.layers.Layer):
    def __init__(self,
        ...
        # Stack GRUCell instances with dropout
        cells = [
            layers.GRUCell(
                units=enc_units,
                dropout=dropout,
        ...
        # Wrap in RNN to get sequences and states
        self.gru = layers.RNN(
            cells,
    ...
    def call(self, tokens, state=None):
        ...
        vectors = self.embedding(tokens)  # (batch, seq_len, embed_dim)

        # outputs_and_states: tuple(output, state1, state2, ...)
        outputs_and_states = self.gru(vectors, initial_state=state)
        output = outputs_and_states[0]           # (batch, seq_len, enc_units)
        states = outputs_and_states[1:]          # list of (batch, enc_units)
        final_state = states[-1]                 # last layer state
...
class Decoder(tf.keras.layers.Layer):
    def __init__(self,
        ...
        # Embedding layer
        self.embedding = layers.Embedding(output_vocab_size,
                                          embedding_dim)
    ...
    def call(self, inputs: DecoderInput, state=None):
        ...
        outputs_and_states = self.gru(vectors, initial_state=state)
        rnn_output = outputs_and_states[0]           # (batch, t, dec_units)
        new_states = outputs_and_states[1:]          # list of (batch, dec_units)
        dec_state = new_states[-1]                   # last layer state
        ...
        # Concatenate and generate logits
        concat = tf.concat([context_vector, rnn_output], axis=-1)  # (batch, t, 2*dec_units)
        attention_vector = self.Wc(concat)                         # (batch, t, dec_units)
        logits = self.fc(attention_vector)                         # (batch, t, output_vocab_size)
