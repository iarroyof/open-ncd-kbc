...
"""
This script is used to get the random prediction object pairs obtained from the val set (). It will perform a clean up and verify that they are valid (based on length)
Afterwards it will shuffle the list and generate a dataframe that will be saved as a tsv. 
"""
...
for num,csv_item in enumerate(csv_files):
    ...
    for idx,inference in inferences_item.iterrows():
        # print('&', csv_nameibs[num]+ ' & ', inference['Obj'], '\\\\' )
    ...
    random.shuffle(obj_true_list)
    # print(pd.DataFrame(zip(obj_list,obj_true_list)))
    pd.DataFrame(zip(obj_list,obj_true_list)).to_csv(csv_item+'object_pairs_val_random.tsv', sep='\t',header=None,index=None)
